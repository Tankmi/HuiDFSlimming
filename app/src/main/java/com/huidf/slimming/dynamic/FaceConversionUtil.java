...
public class FaceConversionUtil {
	...
	private int screenWidth = PreferenceEntity.screenWidth;
	private int screenHeight = PreferenceEntity.screenHeight;
	 ...
	 * @return
	 */
	public SpannableString addFace(Context context, int imgId, String spannableString, int textSize) {
	...
	private void dealExpression(Context context,SpannableString spannableString, Pattern patten, int start)
		...
		while (matcher.find()) {
			...
			if (resId != 0) {
				...
				bitmap = Bitmap.createScaledBitmap(bitmap, (int)(screenWidth*0.097), (int)(screenHeight * 0.052), true);
//				bitmap = Bitmap.createScaledBitmap(bitmap, 200, 200, true);
	...
	private void ParseData(List<String> data, Context context) {
		...
		try {
			...
			int pageCount = (int) Math.ceil(emojis.size() / 5 + 0.1);
