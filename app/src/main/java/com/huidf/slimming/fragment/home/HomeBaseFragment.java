...
import com.huidf.slimming.home.HomeController;
import com.huidf.slimming.home.HomePresenter;
import com.huidf.slimming.manager.RetrofitManager;
import com.huidf.slimming.manager.RxSchedulers;
...
public class HomeBaseFragment extends BaseFragment implements OnClickListener,HomeController.HomeView {
    ...
    @Override
    public void setData(HomeEntity weatherBean) {
        if (weatherBean.code == ContextConstant.RESPONSECODE_200) {
            isInit = true;
            PreferenceEntity.isRefreshHomeData = false;
            Message message = Message.obtain();
            message.what = mHandler.GETSLEEPINFO;
            message.obj = weatherBean.data;
            mHandler.sendMessage(message);
        } else if (weatherBean.code == ContextConstant.RESPONSECODE_310) {    //登录信息过时跳转到登录页
            reLoading();
        } else {
            ToastUtils.showToast(NewWidgetSetting.filtrationStringbuffer(weatherBean.msg, "接口信息异常！"));
        }
    }
    ...
    @Override
    public void setPresenter(HomeController.HomePresenter presenter) {
...
//        if (mComposite != null && mComposite.size()>0) mComposite.clear();  //取消订阅
        if(mPresenter != null){
