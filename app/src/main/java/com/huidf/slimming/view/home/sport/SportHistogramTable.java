...
public class SportHistogramTable extends View {
...
//			colorA = 0x22d7bb;
//			colorB = context.getResources().getColor(R.color.back_sport_his_b);
//			colorC = context.getResources().getColor(R.color.back_sport_his_c);
...
//			canvas.drawText(mScores.get(i).date,newCoordinate + columnarWidth/2, getHeight(), paint_coordinate);	//绘制x轴
	...
	public SportHistogramTable(Context context) {
		super(context);
	}
	public SportHistogramTable(Context context, AttributeSet attrs) {
		super(context, attrs);
		setDetectionData(context,attrs);
	}
	public SportHistogramTable(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void setDetectionData(Context context, AttributeSet attrs) {
		...
		columnarWidth = LayoutUtil.getInstance().getWidgetWidth(30);
		columnarPadding = LayoutUtil.getInstance().getWidgetWidth(150);
		columnarHeight = LayoutUtil.getInstance().getWidgetHeight(210f);
		marginBottom = LayoutUtil.getInstance().getWidgetHeight(80f);
		marginLeft = LayoutUtil.getInstance().getWidgetWidth(30);

		colorA = context.getResources().getColor(R.color.back_sport_his_a);
		colorB = context.getResources().getColor(R.color.back_sport_his_b);
		colorC = context.getResources().getColor(R.color.back_sport_his_c);

		if(mScores == null) mScores = new ArrayList<SportHistogramEntity>();
		mRectf = new RectF();
