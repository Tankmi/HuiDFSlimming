...
//        canvas.drawColor(0xffbbcc00);
    ...
    public void drawShader(Canvas canvas){
        ...
        for (int i = 0; i < score.size(); i++) {
            float data = YCoord(score.get(i).score + "");
            Path path = new Path();
            if (i > 0) {
                float dataLast = YCoord(score.get(i - 1).score + "");
    ...
    public void drawRectf(Canvas canvas) {
        ...
        for (int i = 0; i < score.size(); i++) {
            float data = YCoord(score.get(i).score + "");
            LOG("YCoord(score.get(i).score + \"\"):" + YCoord(score.get(i).score + ""));
            ...
            drawDec(canvas,marginLeft + interval_left_right * (i), data,score.get(i).score + "");
            drawXCoordinateAxis(canvas,i,score.get(i).date + "\n" + score.get(i).date1);
        }
    }
    ...
    public void drawDate(Canvas canvas) {
        ...
        for (int i = 0; i < score.size(); i++) {
            if(i>0){
                canvas.drawLine(marginLeft + interval_left_right * (i-1),YCoord(score.get(i-1).score + ""),
                        marginLeft + interval_left_right * (i), YCoord(score.get(i).score + ""),paint_circle);
            }
        }
    }
